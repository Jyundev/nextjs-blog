...
<Table json='{"headers":["구분","전달 방식","특징"],"rows":[["HTML","비어 있음","JS 실행 전에는 아무것도 표시되지 않음"],["JS 실행 후","Virtual DOM 생성","useEffect로 데이터 요청"],["완성 시점","API 응답 이후","DOM 업데이트 후 렌더링 완료"]]}' />
...
<Table json='{"headers":["장점","단점"],"rows":[["초기 로딩이 매우 빠름 (HTML 즉시 표시)","서버 부하 있음"],["SEO에 강함 (검색엔진이 HTML 읽기 쉬움)","요청마다 렌더링 발생"],["최신 데이터 반영 용이","서버 환경 설정이 필요"]]}' />
