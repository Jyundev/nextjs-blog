...
summary: "루트 경로의 지저분한 설정 파일들 사이에서 소스 코드를 구출하기."
...
app/page.tsx는 실제 화면을 구현하지 않고,
feature의 Page 컴포넌트를 연결하는 역할만 수행한다.
...
프로젝트 규모가 커져도 app 폴더 내부는 파일 몇 개 수준으로 유지되어, 전체적인 라우팅 구조를 파악하는 데 방해를 주지 않는다.
