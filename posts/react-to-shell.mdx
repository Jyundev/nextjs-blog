...
<Table json='{"headers":["구분","정상 동작","React2Shell 취약 상황"],"rows":[["Flight Payload 출처","서버 내부에서 생성","외부 공격자가 조작"],["디코딩 전제","신뢰된 데이터","신뢰 경계 붕괴"],["참조 대상","UI 컴포넌트","서버 내부 코드"],["실행 결과","렌더링","서버 코드 실행"],["보안 영향","없음","RCE"]]}' />
