...
<Table json='{"headers":["상태","설명","동작 방식"],"rows":[["Fresh","최신 데이터로 간주되는 상태","staleTime이 만료되기 전까지 재요청 없이 캐시 데이터 사용"],["Stale","오래된 데이터로 간주되는 상태","화면에는 기존 데이터를 유지한 채 백그라운드에서 재요청 발생"],["Fetching","데이터를 다시 가져오는 중인 상태","네트워크 요청이 진행되며 완료 후 캐시 업데이트"],["Inactive","현재 화면에서 사용되지 않는 상태","gcTime 동안 메모리에 유지되며, 이후 GC 대상이 됨"]]}' />
