...
<Table
  data={{
    headers: ["구분", "AsyncQueue", "await"],
    rows: [
      ["제어 단위", "큐(Queue) 전체", "현재 함수 내부"],
      ["실행 순서 보장", "항상 보장 (전역 큐 기준)", "함수 내부에서만 보장 "],
      ["동시 호출 시 동작", "자동으로 대기 후 순차 실행 ", "병렬 실행됨 (순서 불명확) "],
      ["사용 목적", "여러 비동기 작업 간의 순서 제어", "단일 함수 내의 순차 실행 "],
      ["예시", "여러 컴포넌트/탭에서 동일 리소스 접근", "fetch → 다음 fetch 순서대로 실행"],
